  ...
  const script = `(() => {
    try {
      ...
      var hexToHsl = function(value) {
        var hex = (sanitizeHex(value) || '#3b82f6').slice(1); var r=parseInt(hex.slice(0,2),16)/255,g=parseInt(hex.slice(2,4),16)/255,b=parseInt(hex.slice(4,6),16)/255;
      ...
      if (themeId === 'catppuccin') {
        ...
        var primaryHex = sanitizeHex(readStored('primaryColor') || readCookie('primaryColor')) || '#3b82f6';
